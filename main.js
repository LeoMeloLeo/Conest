    ...
    try {
        ...
        if (error.code = 11000){
            dialog.showMessageBox({
                type: 'error',
                title: 'Atenção!',
                message: "O CNPJ já esta cadastrado\nVerfique se digitou corretamente",
                buttons: ['OK']
            }).then((result) => {
                if (result.response === 0) {
                    //event.reply('reset-form')
...
ipcMain.on('new-product', async (event, produto) => {
    ...
    try {
        ...
        if (error.code = 11000){
            dialog.showMessageBox({
                type: 'error',
                title: 'Atenção!',
                message: "O BARCODE já esta cadastrado\nVerfique se digitou corretamente",
                buttons: ['OK']
            }).then((result) => {
                if (result.response === 0) {
                    //event.reply('reset-form')
    ...
    try {
        ...
        // Confirmação  de cliente  Inserido no banco
        dialog.showMessageBox({
            type: 'info',
            title: 'Aviso',
...
ipcMain.on('update-product', async (event, produto) => {
    ...
    //estrategia se o usuario nao trocou a imagem editar apenas os campos
    if (produto.caminhoImagemPro === "") {
        try {
